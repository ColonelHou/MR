...
package com.hadoop.examples.secondSort;
...
public class SecondarySort {
	 ...
	 * 
	 * SecondarySort就是在对first排序后再对second再排序处理
	 */
	 ...
	 * 定义分区函数类FirstPartitioner，Key的第一次排序。
	 * 这样key中first相同的value会被放在同一个reduce中，在reduce中再做第二次排序 
	 */
	 ...
	 * 
	 * 定义分组函数类FirstGroupingComparator，保证只要key的的第一部分相同，value就进入key的value迭代器中。
	 */
	...
	public static void main(String[] args) throws Exception {
		...
		//为什么没有使用setSortComparatorClass()是因为hadoop对key排序的规则(参看2.Hadoop的key排序逻辑)决定的。
		//由于我们在IntPair中已经定义了compareTo()函数。
